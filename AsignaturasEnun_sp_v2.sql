-- VERSION 2 SIN SELECT - Exception constraint
...
Begin
 ...
 exception 
  -- Capturar la excepción de violación de registro unico (primary key y unique)
  when DUP_VAL_ON_INDEX then
 
    -- Busco la violacion de primary key 
    if SQLERRM like '%PK%' then
        RAISE_APPLICATION_ERROR(-20000, 'La asignatura con id=' || v_idAsignatura || ' ya existe en la titulación ' || v_titulacion || '.');
  
    --Busco la violacion de unique   
    elsif SQLERRM like '%UNQ%' then
        RAISE_APPLICATION_ERROR(-20001, 'La asignatura con nombre=' || v_nombreAsig || ' ya existe en la titulación ' || v_titulacion || '.');
